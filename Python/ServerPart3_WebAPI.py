...
def get_structured_data(req: StudyRequest):
    ...
    cursor.execute("""
        SELECT measurement_name, result_no, display_value
        FROM measurements
        WHERE study_uid = ?
    """, (req.exam.studyUid,))
    ...
    # Build propValues as { "<measurement_name>_<result_no>": display_value }
    prop_values: Dict[str, Any] = {}
    for row in rows:
        key = f"{row['measurement_name'].translate({ord(c): None for c in string.whitespace + ',/.'})}_{row['result_no']}"
